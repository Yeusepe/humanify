...
| Guild config | `GET /guilds/:guildId/policy`, `PUT /guilds/:guildId/policy`, `GET /guilds/:guildId/verification`, `PUT /guilds/:guildId/channels`, `PUT /guilds/:guildId/verification` | all writes create audit records; channel config persists moderator alert plus optional review/audit/log channels, and verification config owns enabled strategy adapters, required proof bundles, face-verification policy, and default provider choices |
...
- verification routes now keep signed session identity explicit while treating concrete adapters as strategy modules:
  ...
  - `POST /guilds/:guildId/verification/sessions` accepts an optional originating `caseId`, creates the canonical verification session immediately, signs the verifier challenge with `challengeId`, `sessionId`, `guildId`, and `userId`, and returns the authoritative guild verification config snapshot needed by the verifier UI
...
- `PUT /guilds/:guildId/verification` now validates and persists guild-level verification configuration against the shared provider and claim-bundle catalogs:
  ...
  - the response surfaces `availableProviderIds`, `enabledProviderIds`, `defaultProviderId`, optional `defaultReusableProofBackendId`, `requiredBundleIds`, and the resolved bundle metadata so the dashboard and verifier can stay adapter-neutral at the app boundary
