...
| Guild config | `GET /guilds/:guildId/policy`, `PUT /guilds/:guildId/policy`, `PUT /guilds/:guildId/channels`, `PUT /guilds/:guildId/verification` | all writes create audit records; channel config persists moderator alert plus optional review/audit/log channels, and verification config owns enabled strategy adapters plus the guild default first-time capture adapter |
