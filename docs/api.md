...
- verification routes now keep signed session identity explicit while treating concrete adapters as strategy modules:
  ...
  - `GET /verification/sessions/:sessionId?token=...` verifies the signed boundary and returns the canonical persisted session/provider state plus the normalized `verification` summary and any persisted `reusableCredentialBridge` when available instead of fabricating completion from browser state
  ...
  - `POST /callbacks/providers/didit` is now live and must verify Didit raw-body webhook signatures, fetch the authoritative Didit decision server-side, reduce it to minimal-custody summary fields, request provider-side deletion after reconciliation, and persist the honest Didit → Privado bridge contract when approved reusable predicates are available
  - that bridge contract is additive API surface on the callback response and `GET /verification/sessions/:sessionId`; it exposes only time-bounded minimal facts (`ageOver18`, `nationality`, `faceVerificationPerformed`, `faceVerificationPassed`), required external issuer inputs, and durable audit refs rather than pretending Humanify minted or stored a full reusable credential
