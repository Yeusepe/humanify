...
| Canonical state | Postgres + `pgvector` | Electric, Rust and Bun services | guilds, members, verification sessions, cases, reports, evidence metadata, outcomes, audit, outbox, receipts |
...
```mermaid
flowchart LR
  A[Discord event / report / provider callback] --> B[Bun validation + normalization]
...
| --- | --- | --- |
| `packages\policy-engine` | score-to-action mapping, verification requirement thresholds, action clamps | call Discord directly or read raw provider payloads |
